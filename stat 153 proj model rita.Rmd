...
```{r}
model2 <- sarima(stock_300$Close, p = 1, d = 0, q = 1, P = 0, D = 1, Q = 2, S = 5)
```
...
```{r}
times = c(186, 196, 206, 216, 226, 236, 246, 256, 266, 276)
sum_squared_errors <- 0
for (timestep in times) {
  train_set <- window(diff_res, end=timestep-0.001)
  test_set <- window(diff_res, start=timestep, end=timestep + 10 - 0.01)
  #
  forecast <- sarima.for(train_set, n.ahead=10, p = 1, d = 0, q = 1, P = 0, D = 1, Q = 2, S = 5)$pred
  #
...
```{r}
mse_model2 = sum_squared_errors / 100
mse_model2
```
