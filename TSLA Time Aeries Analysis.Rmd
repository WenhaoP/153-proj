...
jump <- 10
for (i in start:end) {
train <- window(filtered_stock, end = i - 0.001)
test <- window(filtered_stock, start = i, end = i + 10 - 0.01)
m1_forecast <- forecast(ets(train), h = 5)
sum_squared_errors[] = sum_squared_errors[] + sum((m1_forecast[['mean']]-test)^2)
}
