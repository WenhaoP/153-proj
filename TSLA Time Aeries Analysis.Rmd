...
```{r, fig.cap='Forecast of Tesla Stock in the next Ten Days'}
model_forecast <- sarima.for(stock_300$Close, n.ahead = 10, p = 1, d = 2, q = 1, 
                        P = 0, D = 0, Q = 0, S = 0)
```
Figure . Forecast of TSLA close price.

The time plot above shows the predicted value of TSLA stock price for the next ten days of operation, from November 1 and November 12, 2021. 
The black points are the historical data, and the red points are the prediction value, with gray areas of 68% or 95% prediction intervals, respectively. With the standard error generated by the model, we listed the prediction intervals (with 2 standard error bands) in the following table to capture the uncertainty of each prediction point.
Our model predicts an exponential trend of the future stock prices. 
...
```{r}
stock_new = read.csv("/Users/charlottewang/Desktop/TSLA.csv")
...
df
```
Table : This dataframe shows our forecast of the next ten points, as well as the true Tesla stock close price during the analyzed period. The last two columns display the prediction intervals for the next ten points applied by model 3. In the numerical statistics table, we choose 95% prediction interval to estimate the band in which a future observation will fall.

```{r, fig.cap="Theoretical and Actual Value of Tesla Stock"}
plot(1:10, xlab = "Time", ylab = "Stock Price ($)", ylim=c(1000,1550),
     main = "Theoretical and Actual Value of Tesla Stock",
     sub = "(Nov 1, 2021 - Nov 12, 2021)",
...
#points(1:10, df[ ,2], type = "n", pch = 19, col = "red")
...
```
Figure .
The figure above displays the comparison between the forecasted and actual values of Tesla stock price in the analyzed period. With the 95% prediction interval, it is shown that our model fits six out of ten days appropriately; however, since Elon Musk sold 10% of his Tesla holding on November 7, its share price sharply decreased, which lead to a significant deviation from the predicted value.
